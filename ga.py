...
def mutate(x):
    ...
    while True:
        ...
        # i = (np.random.choice(int(len(offspring)/7)))*7 # random schedule for change
        charger_code, connector_id, track_code, start_time, end_time, ampere, price = offspring.position[i: i+7]
        # charger_code, connector_id, track_code, start_time, end_time, ampere, price = offspring[i: i+7]
        ...
        if end_time-start_time >= time: # if the time is shorter than befor - the time is good for the charger
            ...
            offspring.position[i+4] = end_time
            offspring.position[i+5] = _ampere
            offspring.position[i+6] = price
            # offspring[i+4] = start_time+time
            # offspring[i+5] = _ampere
            # offspring[i+6] = price
            break
        else:
            for j in range(0, len(offspring.position), 7):
                charger_code, connector_id, track_code, start_time, end_time, ampere, price = offspring.position[j: j+7]
            # for j in range(0, len(offspring), 7): # for each charger - the chargings list 
            #     charger_code, connector_id, track_code, start_time, end_time, ampere, price = offspring[j: j+7]
                chargers_busy[(charger_code, connector_id)].append({"start": start_time, "end": end_time})
                ...
                if sch['start'] == start_time: # if i found the current schedule
                    if j+1 == len(chargers_busy[(charger_code, connector_id)]): # if there are no more schedules on the achargers after this - i can extend the charging time
                        ...
                        if next_start - (start_time+time) >=  600000: # if i have enough time before the next schedule - i can extend this charging
                            ...
                            found = False
                            for c in range(len(chargers_busy)):
                                if chargers[c]['ampere']>=_ampere:
                                    charger = chargers_busy[c]
                                    charger.sort(key=lambda x: x['strat'])
                                    start = 0
